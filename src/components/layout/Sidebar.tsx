...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
    ...
    return (
        <>
            ...
            {/* Sidebar Container */}
            <div className={cn('fixed top-0 left-0 z-40 h-screen flex', mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
                {/* Toggle Button - Always visible on desktop */}
                <button
                    onClick={onToggle}
                    className={cn(
                        'hidden lg:flex absolute z-50 items-center justify-center w-10 h-10 rounded-lg bg-white border border-slate-200 shadow-sm hover:bg-slate-50 transition-all duration-300',
                        collapsed ? '-right-12 top-4' : '-right-12 top-4'
                    )}
